...
function runIntegrationTests() {
  ...
  results.push(
    runTest("Charles Traits", () => {
      const charles = PersonaRegistry.get("charles");
      return (
        charles !== undefined &&
        charles.traits.personality.includes("British") &&
        charles.traits.expertise.some((e) => e.includes("YAGNI"))
      );
    })
  );
  ...
  results.push(
    runTest("System Prompt Enhancement", () => {
      const charles = PersonaRegistry.get("charles");
  ...
  results.push(
    runTest("User Prompt Enhancement", () => {
      const ada = PersonaRegistry.get("ada");
  ...
  results.push(
    runTest("Unique Persona Behaviors", () => {
      const charles = PersonaRegistry.get("charles");
      const ada = PersonaRegistry.get("ada");
      ...
      const charlesVersion =
        charles?.enhanceSystemPrompt(prompt, context) || "";
      const adaVersion = ada?.enhanceSystemPrompt(prompt, context) || "";

      return (
        charlesVersion !== adaVersion &&
  ...
  results.push(
    runTest("Context-Aware Enhancement", () => {
      const charles = PersonaRegistry.get("charles");
      ...
      const reviewEnhanced =
        charles?.enhanceSystemPrompt(basePrompt, reviewContext) || "";
      const researchEnhanced =
        charles?.enhanceSystemPrompt(basePrompt, researchContext) || "";

      // Different contexts should produce different prompts
      return (
        reviewEnhanced !== researchEnhanced &&
        reviewEnhanced.toLowerCase().includes("wrong") && // Review: "What's actually wrong..."
        researchEnhanced.toLowerCase().includes("simplest")
      ); // Research: "Find the simplest..."
    })
  );

  // Test 8: Catchphrases present
  results.push(
    runTest("Persona Catchphrases", () => {
      const charles = PersonaRegistry.get("charles");
      const ada = PersonaRegistry.get("ada");

      return (
        charles?.traits.catchphrases !== undefined &&
